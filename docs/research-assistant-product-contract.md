...
- F-04A requires the host Agent to declare the Claim identity, current/proposed frontmatter, desired lifecycle status, and any explicit conflict identity plus Claim/Result membership. Core validates all five Schema v2 statuses, immutable F-03 identity, strictly advancing `updated_at`, preservation of the most recent real verification time across non-verified transitions, and deterministic audit fingerprints; it does not decide whether a transition is scientifically justified. Display title is not identity: a coordinated rename may preserve the same F-03 entity ID when the proposed title matches the current registry entity. The complete-frontmatter change report is not authorization for a writer to persist unrelated ownership, Source, Evidence, or body changes.
- A target `verified` transition requires a matching current F-02B result for the exact Claim frontmatter revision, project/path/Source/Evidence bindings, and `last_verified_at == updated_at`; replaying a proof after any frontmatter edit fails closed. A target `conflicting` transition is accepted only with a bound explicit coexistence proof that contains the exact proposed Claim revision and retains at least two distinct Claims and at least two distinct Results overall. The same Result may be referenced by multiple competing interpretations, and Result pages need not themselves be `conflicting`. Opposing Evidence or relation labels never infer conflict, and no winner or replacement is selected.
- F-04A is read-only and in-memory: it does not persist status, write Markdown/registries, propagate stale state, reopen Sources, read research binaries, or add CLI/MCP/Web behavior. F-04 remains partial until controlled persistence and product integration land with F-05 and later units.
- F-05A is a pure in-memory planner over exact caller-supplied current/proposed Knowledge Schema v2 bytes. It enforces immutable project/type/ownership/creation time, strictly advancing `updated_at`, whole-body `generated`/`user` ownership, and explicit LF/CRLF-delimited protected user regions for `mixed` pages. Regeneration restores exact user-region content from the caller-supplied base snapshot; user edits cannot change the generated skeleton. Unicode separators are content, not hidden marker boundaries.
- F-05A protects Markdown body ownership only. Its frontmatter change report, `plan_id`, and `validation_complete` are not authorization for scientific status, Evidence, Source, title, lifecycle changes, or persistence; the host must compose F-02/F-03/F-04 and user authorization. F-05B then reloads the registration and live page under `machine-state.lock`, independently recomputes F-05A, requires an exact structured host/actor/session/decision authorization, performs a stable-object exact-revision recheck plus non-clobbering create/atomic replace, verifies the output hash, and appends a bounded body-free `prepared -> committed | conflict | failed | commit-unknown` audit transaction. Authorization and trusted-decision replay fail closed; a dangling `prepared` record blocks later writes. F-05B does not create Markdown directories, decide semantics, read Source/binary content, send externally, or expose CLI/MCP/Hook/Web/`ResearchCoreService` behavior.

Detailed machine contracts: [`knowledge-artifact-contract.md`](knowledge-artifact-contract.md), [`claim-evidence-currentness.md`](claim-evidence-currentness.md), [`research-relations.md`](research-relations.md), [`claim-lifecycle.md`](claim-lifecycle.md), and [`controlled-markdown-writes.md`](controlled-markdown-writes.md).
...
As of 2026-07-19, the deterministic `project understand` pipeline, independent B-07 reading priority, H-04 event-input boundary, and H-07 conservative reconciliation boundary are integrated. E-08 `--open` produces a self-contained local read-only run page with no external resources, while the completed J-03/J-04 product research cockpit is a separate loopback-only surface for registered projects, all 15 canonical Knowledge entries, coverage and file state, Evidence/source/locator traces, plans, tasks, and run history. It remains read-only by default; an explicit trusted local session may edit only the fixed Goal, backlog, project-status, and user-confirmed-conclusion mixed regions through exact F-05A/F-05B compare-and-swap and body-free audit. The repository development-supervision dashboard remains a distinct engineering surface. I-01 through I-04 provide strict Goal/task state, a current project snapshot, and a non-executable DRAFT initial plan that the E-07 renderer projects through the same controlled-write boundary. J-01 now accepts the full authorized R3-minus-C-07 fixture chain twice from clean temporary workspaces, including exact Source/Evidence/Locator fidelity, H-07 reconciliation, narrowly trusted stale-state refresh, DRAFT planning, and controlled rendering. Verified Query and mature planning remain later work; Query is still unavailable until G-04, C-07 remains `deferred/not_started`, and R4 has not started.
