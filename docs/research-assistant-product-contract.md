...
- F-05A protects Markdown body ownership only. Its frontmatter change report, `plan_id`, and `validation_complete` are not authorization for scientific status, Evidence, Source, title, lifecycle changes, or persistence; the host must compose F-02/F-03/F-04 and user authorization, and F-05B must freshly rebind the live base/proposal before exact CAS. F-05A performs no filesystem write, lock, audit persistence, Source/binary read, external send, or CLI/MCP/Web operation. F-05 remains partial until F-05B persistence/audit.
