...
As of 2026-07-18, the deterministic `project understand` pipeline, independent B-07 reading priority, H-04 event-input boundary, and H-07 conservative reconciliation boundary are integrated. E-08 `--open` produces only a self-contained local read-only run page with no external resources; it does not present the legacy development supervision dashboard as the product cockpit. I-02 now provides the strict, non-self-certifying task protocol and read-only legacy Todo compatibility. State snapshots, first-run Goal/Task/plan integration, the product Web cockpit, and controlled Web editing remain the subsequent R3 I/J units.
