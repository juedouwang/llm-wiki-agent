...
Readers and writers serialize through an operating-system exclusive lock on the persistent adjacent `sources.jsonl.lock` file. Lock-file existence is not ownership. Ordinary readers retain that lock through their complete stable read and parse so a Windows read-only descriptor cannot deny a concurrent atomic replacement outside the protocol. One pinned machine-state-root lease is retained across lock acquisition, under-lock load or reload, equality comparison, atomic replacement, and any rollback, so concurrent synchronizations cannot allocate multiple IDs for the same path or redirect later stages to a substituted root. Operations that also need Evidence acquire the Source lock before the Evidence lock. Redirected paths, changed root/lock/registry identity, post-replace uncertainty, or visible-but-not-fully-durable replacement fail closed with an explicit commit-state error rather than a false success.
