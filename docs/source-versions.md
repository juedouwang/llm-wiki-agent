...
Writers retain the D-01 adjacent lock and atomic same-directory replacement protocol. Concurrent writers reload under the exclusive lock, so one changed Manifest generation appends exactly one version. A Manifest older than any persisted observation is rejected rather than overwriting newer state. Failed validation leaves the existing registry bytes untouched and creates no partial replacement.
