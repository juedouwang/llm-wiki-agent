...
- E-05 research-linkage validation: `tests/test_research_linkage.py`
...
| `research_linkage(project_id, observations=...)` | `tools.research_linkage.generate_research_linkage` over explicit host entities/relations, the exact current Manifest, and current-project Evidence referential integrity | local/path-bearing `ResearchLinkageResult` |
...
linkage = core.research_linkage(
    registration.project_id,
    observations=[
        ResearchEntityObservation(
            "paper", "paper", "Method paper", "papers/method.md", "metadata"
        ),
        ResearchEntityObservation(
            "implementation", "implementation", "Model", "src/model.py",
            "implementation"
        ),
        ResearchRelationObservation(
            "implementation", "paper", "implements-method-from",
            "implementation"
        ),
    ],
)
...
E-05 keeps host-declared scientific semantics separate from deterministic Core
validation. It supports paper, method, innovation, dataset, configuration,
implementation, claim, experiment, result, and unknown entities plus bounded
lowercase kebab-case directed relation labels. Inference entities and relations
cannot be labelled `observed`; metadata fallback rows remain uncertain
candidates and create no semantic relation.

Every referenced Evidence ID must exist in the current project's real Evidence
registry. This is registry referential integrity, not F-02B Source-version,
Locator, excerpt, stance, or Claim-currentness validation. The current loader
uses the Source/Evidence lock order outside `machine-state.lock`, then rechecks
the exact Manifest and artifact under `machine-state.lock`. E-05 opens no Source
bytes, writes no curated Markdown, and adds no CLI, MCP, Hook, or Web operation.
See [`research-linkage.md`](research-linkage.md).
...
F-05A's controlled Markdown planner and F-05B's bounded persistence/audit layer
are standalone Core primitives rather than service or transport operations. F-05A
validates exact caller-supplied current bytes, current Schema v2/path identity,
timestamps, and explicit generated/user/mixed body ownership. F-05B reloads the
registered live page under `machine-state.lock`, independently recomputes that plan,
requires exact structured host/session authorization, applies the stable-file
exact-revision publication boundary, and records a canonical body-free two-phase
audit. Neither primitive decides scientific/frontmatter semantics. They add no
