...
13. F-05A defines only the deterministic, in-memory controlled Markdown plan in `tools/controlled_markdown.py`. It binds a proposal to exact caller-supplied current bytes, strict Knowledge Schema v2/path identity, immutable project/type/ownership/creation time, a strictly advancing `updated_at`, and explicit body ownership. `generated` and `user` pages have whole-body ownership; `mixed` pages use bounded LF/CRLF-delimited `llmwiki:user-region` markers, preserve exact user-region bytes during regeneration, and forbid user edits from changing the generated skeleton. This is body protection, not semantic authorization: callers must separately compose F-02/F-03/F-04 and user decisions for frontmatter changes. F-05A performs no filesystem write, lock, audit persistence, Source/binary read, external send, or CLI/MCP/Web operation. F-05 remains partial until F-05B adds bounded persistence and audit. See `docs/controlled-markdown-writes.md`.
