...
14. I-01 defines the strict Goal/Milestone Schema v1 in `tools/research_goals.py`: machine state is `.llmwiki/projects/<project_id>/indexes/goals.json`, the human projection is Knowledge Schema v2 `goals.md`, and `draft` records missing onboarding information without implying user confirmation. Parsing rejects strict-UTF-8/duplicate-key/unknown/future/malformed data and project mismatches; registered machine writes use `machine-state.lock` and stable atomic access. I-01 does not bypass F-05A/F-05B to persist curated Markdown, infer goals or confirmation, expose Web/MCP/Hook behavior, or read C-07 binaries. See `docs/goals-and-milestones.md`.
15. I-02 defines the strict task protocol in `tools/research_tasks.py`: current tasks require `why_now`, inputs, project-relative allowed scope, DoD, verification, and artifacts; canonical Evidence IDs, same-collection acyclic dependencies, and explicit completion references are validated. Machine state is `.llmwiki/projects/<project_id>/indexes/tasks.json`; the optional human projection is Knowledge Schema v2 `plans/backlog.md` with the bounded `user-backlog` region. Legacy Todo aliases are explicit read-only compatibility and remain non-executable draft views; they are never silently serialized or migrated. TaskStore writes only registered machine state under `machine-state.lock`, does not write curated Markdown, infer authorization/completion, open source-project content, or process C-07 binaries. See `docs/task-protocol.md`.
