...
B-03 established the accountable directory ledger: every in-scope regular file is recorded regardless of format, excluded files and pruned-directory boundaries remain reconcilable, and symbolic links use B-02 root/cycle/duplicate checks. B-04 adds scan generations and conservative local SHA-256 reuse. B-05 writes `project-inventory-v3` and deterministically classifies every ordinary file by format, language, and research role with auditable reasons; unknown files remain explicit. A bounded classification prefix may be read only when B-02 grants local raw-content access. Sensitive and oversized files must remain path-only after the B-04 hash; samples must not be persisted, sent externally, passed to an LLM, or written back to the source project. Do not add final processing states, extraction, Evidence, MCP, Hook, or Web behavior to this inventory step. See `docs/project-inventory.md`, `docs/file-fingerprints.md`, and `docs/file-classification.md`.
